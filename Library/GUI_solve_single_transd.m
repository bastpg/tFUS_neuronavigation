...
% imperative that the normals points toward the brain!!!
% input 
...
if display == 1
    figure(FIG2);
    subplot(2,2,4);  imagesc( squeeze(abs(PP(:,round(end/2),:))) ); colorbar; axis image off; title('Pressure');
end

Raff_inv = [ R   T  ;
            0   0   0   1];

I_INT = reshape( interpAffineGrid_MEX( mgrid.x , mgrid.y , mgrid.z , abs(PP).^2 , x , y , z , Raff_inv ) , [ numel(x) numel(y)  numel(z) ] );
